...
func NewWithLogger(config ClientConfig, transport http.RoundTripper, logger Logger) (*Client, error) {
	...
	// We always use TLS1.3 for transport with libcurl
	easy.Setopt(curl.OPT_SSLVERSION, curl.SSLVERSION_TLSv1_3)
	// TODO set TLS KEM Curves ID now, need change to parameter from user later
	easy.Setopt(curl.OPT_CURVES, "kyber768")
	// set verbose mode now
...
func write_header(ptr []byte, userdata interface{}) bool {
        (*userdata.(*[]string)) = append((*userdata.(*[]string)), string(ptr))
        return true
}

func write_data(ptr []byte, userdata interface{}) bool {
	(*userdata.(*[]byte)) = append((*userdata.(*[]byte)), ptr...)
        return true
}
...
//========
	...
	easy.Setopt(curl.OPT_HEADERFUNCTION, write_header)

	// write response headers
	var resp_header []string
	easy.Setopt(curl.OPT_HEADERDATA, &resp_header)

        // write response body
	var resp_body []byte
        easy.Setopt(curl.OPT_WRITEDATA, &resp_body)
	...
	if req.Body != nil {
		...
		if err := easy.Perform(); err != nil {
			fmt.Println("failed with request body ", err.Error())
		}
		...
		if err := easy.Perform(); err != nil {
                        fmt.Println("failed without request body ", err.Error())
                }
	}

	httpcode, err := easy.Getinfo(curl.INFO_HTTP_CODE)
	contentlen, err := easy.Getinfo(curl.INFO_CONTENT_LENGTH_DOWNLOAD)


	// create a http response from libcurl return
	response := &http.Response{
		StatusCode: httpcode.(int),
		Proto: "HTTP/1.1",
		ContentLength: (int64)(contentlen.(float64)),
		Body: ioutil.NopCloser(bytes.NewBuffer(resp_body)),
		Request: req,
		Header: make(http.Header, 0),
	}

	// fill response headers from libcurl return
	for _, v := range resp_header {
                h := strings.Split(v, ":")
                if len(h) == 2 {
			response.Header.Set(strings.TrimSpace(h[0]), strings.TrimSpace(h[1]))
                }
        }
